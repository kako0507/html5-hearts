define(['ui', 'Human', 'Ai', 'board', 'config', 'jquery', 'rules', 'RandomBrain', 'AsyncBrain', 'SimpleBrain', 'PomDPBrain'],
  (ui, Human, Ai, board, config, $, rules, RandomBrain, AsyncBrain, SimpleBrain, PomDPBrain) => {
    let rounds = 0;
    const players = [
    ...
    const informCardOut = function (player, card) {
      if (card.suit === 1) {
        heartBroken = true;
      }
      players.forEach((p) => {
    ...
    const adds = [1, 3, 2];
    const getPlayerForTransfer = function (id) {
      ...
      proceed() {
        ({
          ...
          endRound() {
            ...
            rank.sort((a, b) => a._oldScore - b._oldScore);
            rank.forEach((r, ind) => {
              r.display.rank = ind;
            });
            players.forEach((p) => {
            ...
            if (players.some(p => p._oldScore >= 100)) {
              players.forEach((p) => {
